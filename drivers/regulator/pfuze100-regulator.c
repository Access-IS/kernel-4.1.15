...
#define FORCE_OVERRIDE_VLDOCTRL_REGFIX
#ifdef FORCE_OVERRIDE_VLDOCTRL_REGFIX
	if(pfuze_chip->chip_id == PFUZE3001)
    {
        (void)regmap_write(pfuze_chip->regmap, PFUZE100_VGEN2VOL, 0x1e );
        (void)regmap_write(pfuze_chip->regmap, PFUZE100_VGEN5VOL, 0x1a );
        (void)regmap_write(pfuze_chip->regmap, PFUZE100_VGEN6VOL, 0x10 );
    }
#endif
